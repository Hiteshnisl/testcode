...
import { IntlTelInputDirective } from 'src/app/intl-tel-input.directive';
...
export class EditClientProfileComponent implements OnInit, OnDestroy {
  ...
  selectedCompanyPhone: any;
  selectedContactPhone: any;
  ...
  fatchDetail() {
    ...
    this._clientProfileService.clientEditForm.patchValue({
      ...
      companyPhoneCountryCode: this.client.companyPhoneCountryCode,
      contactPhone: this.client.contactPhone,
      contactPhoneCountryCode: this.client.contactPhoneCountryCode,
    ...
    this.setPhoneCountryCode(this.client.companyPhoneCountryCode || '','companyPhone', this.client.companyPhone);
    this.setPhoneCountryCode(this.client.contactPhoneCountryCode || '','contactPhone', this.client.contactPhone);
  ...
  setPhoneCountryCode(countryCode: string, type, phoneNumber) {
    const phoneInput = type === 'contactPhone'? this.el.nativeElement.querySelector('#contactPhone') : this.el.nativeElement.querySelector('#companyPhone');
      if (phoneInput) {
          this.intlTelInputDirective.setCountryCodeForElement(phoneInput, countryCode);
      }
      const formattedPhoneNumber = phoneNumber.replace(/^0+/, '');
      phoneInput.value = formattedPhoneNumber;
  }
  
  isCompanyPhoneInvalid(): boolean {
    const control = this._clientProfileService.clientEditForm.get('companyPhone');
    return control.hasError('invalidPhoneNumber') && control.touched;
  }
  isContactPhoneInvalid(): boolean {
    const control = this._clientProfileService.clientEditForm.get('contactPhone');
    return control.hasError('invalidPhoneNumber') && control.touched;
  }

  onCompanyPhoneCountryChange(event: any): void {
    this.selectedCompanyPhone = event;
    console.log('this.selectedCompanyPhone', this.selectedCompanyPhone);
  }
  onContactPhoneCountryChange(event: any): void {
    this.selectedContactPhone = event;
    console.log('this.selectedContactPhone', this.selectedContactPhone);
  }
  onSubmit(next?: string) {
    this.submitted = true;
    const companyPhoneCountryCode = this.selectedCompanyPhone ? this.selectedCompanyPhone.iso2 : '';
    this._clientProfileService.clientEditForm.controls['companyPhoneCountryCode'].setValue(companyPhoneCountryCode);
    const contactPhoneCountryCode = this.selectedContactPhone ? this.selectedContactPhone.iso2 : '';
    this._clientProfileService.clientEditForm.controls['contactPhoneCountryCode'].setValue(contactPhoneCountryCode);
    console.log('this._clientProfileService.clientEditForm', this._clientProfileService.clientEditForm)
